...
void main() async {
  ...
  int age = box.get('age');
  developer.log('=== 讀取初始資料 ===');
  developer.log('名字: $name, 年齡: $age'); // 輸出: Gemini, 25

  // 【U - Update】更新資料 (重複使用相同的 Key 即可覆蓋)
  await box.put('age', 26);
  developer.log('=== 讀取更新後的資料 ===');
  developer.log('新年齡: ${box.get('age')}'); // 輸出: 26

  // 額外技巧：如果找不到 Key，可以設定預設回傳值 (defaultValue)
  var hobby = box.get('hobby', defaultValue: '未填寫');
  developer.log('嗜好: $hobby'); // 輸出: 未填寫

  // 【D - Delete】刪除資料
  await box.delete('name');
  developer.log('=== 刪除後的檢查 ===');
  developer.log('名字還在嗎？: ${box.get('name')}'); // 輸出: null
